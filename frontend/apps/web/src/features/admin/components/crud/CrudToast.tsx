...
import type { CrudToastState } from "./types";

export interface CrudToastProps {
  onDismiss: () => void;
  toast: CrudToastState | null;
}

export const CrudToast = ({ onDismiss, toast }: CrudToastProps) => {
  useEffect(() => {
    if (!toast) return;
  ...
  return createPortal(
    <div
      className={`lms-organization-toast ${toast.tone === "error" ? "is-error" : "is-success"}`}
      role="status"
      aria-atomic="true"
      aria-live="polite"
    >
